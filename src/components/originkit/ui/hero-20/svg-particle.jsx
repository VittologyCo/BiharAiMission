...
function OriginkitBaseParticleImage(__props) {
  ...
  const initParticles = () => {
    ...
    const tryLoad = (cors) => {
      ...
      img.onload = () => {
        ...
        const iH = img.naturalHeight || img.height || 675;
        ...
        if (canvasAspect > imgAspect) {
          ...
          rh = W / imgAspect;
          rx = 0;
          ry = H - rh; // Anchor temple to bottom of stage
        } else {
          rh = H * 1.05;
          rw = rh * imgAspect;
          rx = (W - rw) / 2;
        ...
        for (let y = 0; y < H; y += gap) {
          for (let x = 0; x < W; x += gap) {
            ...
            // Filter out pale outdoor sky so only the architectural Nalanda ruins generate particles
            const isSky =
              (r > 180 && g > 185 && b > 195) ||
              (lum > 195 && Math.abs(r - b) < 30);
            if (isSky) continue;
  ...
  // ── Render loop with automatic IntersectionObserver off-screen pausing ────
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
